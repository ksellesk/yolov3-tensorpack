...
from model_fpn import (fpn_model, multilevel_roi_align, multilevel_rpn_losses,
                       generate_fpn_proposals)
from model_cascade import CascadeRCNNHead
from model_box import (clip_boxes, crop_and_resize, roi_align, RPNAnchors)

from data import (get_train_dataflow, get_eval_dataflow, get_all_anchors,
                  get_all_anchors_fpn)
from viz import (draw_annotation, draw_proposal_recall, draw_predictions,
                 draw_final_outputs)
from eval import (eval_coco, detect_one_image, print_evaluation_scores,
                  DetectionResult)
...
class DetectionModel(ModelDesc):
    ...
    def optimizer(self):
        lr = tf.get_variable(
            'learning_rate', initializer=0.003, trainable=False)
...
class ResNetC4Model(DetectionModel):
    def inputs(self):
        ret = [
            ...
            tf.placeholder(tf.int32, (None, None, cfg.RPN.NUM_ANCHOR),
                           'anchor_labels'),
            tf.placeholder(tf.float32, (None, None, cfg.RPN.NUM_ANCHOR, 4),
                           'anchor_boxes'),
        ...
        if cfg.MODE_MASK:
            ret.append(
                tf.placeholder(tf.uint8, (None, None, None),
                               'gt_masks'))  # NR_GT x height x width
        return ret
...
class ResNetFPNModel(DetectionModel):
    def inputs(self):
        ret = [tf.placeholder(tf.float32, (None, None, 3), 'image')]
...
def visualize(model, model_path, nr_visualize=100, output_dir='output'):
    ...
    pred = OfflinePredictor(
        PredictConfig(
            ...
            output_names=[
                'generate_{}_proposals/boxes'.format('fpn' if cfg.
                                                     MODE_FPN else 'rpn'),
                'generate_{}_proposals/scores'.format('fpn' if cfg.
                                                      MODE_FPN else 'rpn'),
                'fastrcnn_all_scores',
                'output/boxes',
                'output/scores',
...
class EvalCallback(Callback):
    ...
    def _build_coco_predictor(self, idx):
        graph_func = self.trainer.get_predictor(
        ...
        if cfg.TRAINER == 'replicated':
            ...
            if self._horovod_run_eval:
                ...
                output_partial = os.path.join(
                    logdir, 'outputs{}-part{}.json'.format(
                        self.global_step, hvd.local_rank()))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--logdir', help='log directory', default='train_log/maskrcnn')
    parser.add_argument(
        '--visualize',
        action='store_true',
    ...
    if args.visualize or args.evaluate or args.predict:
        ...
        for idx, steps in enumerate(cfg.TRAIN.LR_SCHEDULE[:-1]):
            mult = 0.1**(idx + 1)
            lr_schedule.append((steps * factor // stepnum,
                                cfg.TRAIN.BASE_LR * mult))
        ...
        logger.info(
            "Total passes of the training set is: {}".format(total_passes))
